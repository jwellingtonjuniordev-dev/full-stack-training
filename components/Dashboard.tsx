...
function ExtratoSaldo({ total }: ExtratoSaldoProps) {
  return <p>Rendimento total: {total.toFixed(2)}€</p>;
}
...
function Dashboard() {
  ...
  return (
    <div>
      <h1>Dashboard</h1>
      <ExtratoSaldo total={calculoDeRendimento} />
      <ul>
        {transacoes.map((item) => (
          <li
            key={item.id}
            style={{ color: item.tipo === "entrada" ? "green" : "red" }}
          >
      ...
      <select
        value={tipo}
        onChange={(e) => novoTipo(e.target.value as "entrada" | "saida")}
      >
      ...
      <button onClick={lidarComAdicao}>Adicionar Transação</button>
    </div>
  );
}
